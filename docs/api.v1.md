...
| `years` | array[integer] | Нет | Список годов для фильтрации |
...
**Примеры использования параметра `years`:**

- **Фильтр по одному году:** `?years=2023`
- **Фильтр по нескольким годам:** `?years=2022&years=2023&years=2024`
- **Без фильтра по году:** параметр `years` не передается

**Примечание:** Фильтр по годам работает по принципу "ИЛИ" - возвращаются компании, у которых год соответствует любому из указанных в списке годов.

**Примеры запросов:**

1. **Фильтр по одному году:**

   ```
   GET /api/v1/companies/filter?years=2023
   ```

2. **Фильтр по нескольким годам:**

   ```
   GET /api/v1/companies/filter?years=2022&years=2023&years=2024
   ```

3. **Комбинированный фильтр с годами:**

   ```
   GET /api/v1/companies/filter?main_industry=IT&years=2023&years=2024&support_measures=true
   ```

4. **Фильтр без указания года:**
   ```
   GET /api/v1/companies/filter?spark_status=Действующая&company_size_final=Крупное
   ```
